...
import { COLONNES_DEPLACEMENT, SITE_AERIEN_DDL } from './site-aerien-schema';
 ...
 *
 * **Fichier figé.** Les appareils déjà installés n'ont pas de `user_version` et peuvent se trouver à
 * n'importe quel état historique : cette étape unique les ramène au schéma courant par introspection
 * (`PRAGMA table_info` + `ALTER TABLE ADD COLUMN`), une dernière fois. Un installation neuve passe
 * aussi par ici : `CREATE TABLE IF NOT EXISTS` crée tout, l'introspection ne trouve rien à ajouter.
 *
 * Tout changement de schéma postérieur est une nouvelle étape numérotée dans `migrations-captures.ts`
 * (SQL direct, sans introspection) : on n'ajoute rien à ce fichier.
 */
...
export async function appliquerBaseline(db: SQLite.SQLiteDatabase): Promise<void> {
  await creerTables(db);
  await db.execAsync(SITE_AERIEN_DDL);
  await db.execAsync(VOL_DDL);
  await db.execAsync(STOCK_DDL);
  await rattraperColonnes(db);
}
...
async function rattraperColonnes(db: SQLite.SQLiteDatabase): Promise<void> {
  ...
  // Saisies de vol et de site aérien (#643, #644), auparavant migrées par referentiel-db.
  await ajouterColonnesManquantes(db, 'site_aerien_deplacement', colonnesDeplacement);
  await ajouterColonnesManquantes(db, 'vol', colonnesVol);
}

const colonnesVol = COLONNES_VOL_FACULTATIVES;
const colonnesDeplacement = COLONNES_DEPLACEMENT;
