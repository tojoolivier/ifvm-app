    ...
    for col_name, col_type in columns_to_add_infestation:
        ...
        sa.text("""
            SELECT EXISTS (
                SELECT 1 
                FROM pg_constraint 
                WHERE conname = 'ck_prospection_capture_stade'
            )
        """)
    ).scalar()
    
    if result:
        op.execute("ALTER TABLE prospection_capture DROP CONSTRAINT ck_prospection_capture_stade")
...
def downgrade() -> None:
    ...
    for col in ['type_larve', 'surface_contaminee_ha', 'interdistance_m', 'nb_taches_bandes',
                ...
                'taille_large', 'taille_long', 'pullulation_nb']:
        try:
