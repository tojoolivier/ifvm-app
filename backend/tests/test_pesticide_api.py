...
# --- type_produit (choc / barrière) — migration 0044 --------------------------------


@pytest.mark.asyncio
async def test_create_pesticide_type_produit(client: AsyncClient, auth_headers: dict):
    response = await client.post(
        "/pesticides",
        json=payload_pesticide(type_produit="produit_choc"),
        headers=auth_headers,
    )

    assert response.status_code == 201
    assert response.json()["type_produit"] == "produit_choc"


@pytest.mark.asyncio
async def test_create_pesticide_type_produit_optionnel(client: AsyncClient, auth_headers: dict):
    """Nullable : les pesticides déjà enregistrés n'ont pas cette classification."""
    response = await client.post("/pesticides", json=payload_pesticide(), headers=auth_headers)

    assert response.status_code == 201
    assert response.json()["type_produit"] is None


@pytest.mark.asyncio
async def test_create_pesticide_type_produit_invalide(client: AsyncClient, auth_headers: dict):
    """422 Pydantic lisible plutôt que l'IntegrityError du CHECK `ck_pesticide_type_produit`."""
    response = await client.post(
        "/pesticides",
        json=payload_pesticide(type_produit="autre_chose"),
        headers=auth_headers,
    )
...
@pytest.mark.asyncio
async def test_update_pesticide_type_produit(client: AsyncClient, auth_headers: dict, pesticide):
    response = await client.put(
        f"/pesticides/{pesticide.id}",
        json={"type_produit": "produit_barriere"},
        headers=auth_headers,
    )

    assert response.status_code == 200
    assert response.json()["type_produit"] == "produit_barriere"


@pytest.mark.asyncio
async def test_update_pesticide_type_produit_invalide(
    client: AsyncClient, auth_headers: dict, pesticide
):
    response = await client.put(
        f"/pesticides/{pesticide.id}",
        json={"type_produit": "autre_chose"},
        headers=auth_headers,
    )
